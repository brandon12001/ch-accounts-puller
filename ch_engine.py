...
# Page rendering for OCR. pypdfium2 ships its own binary in the wheel, so it
# works with no apt packages at all. pdf2image needs poppler-utils installed
# via packages.txt and is kept only as a fallback.
try:
...
# Kept for backwards compatibility with anything checking this flag.
HAS_OCR = HAS_TESS and (HAS_PDFIUM or HAS_PDF2IMAGE)
try:
